...
def learn_from_groundtruth_shared(input_shape, my_specs, lr):
    sc = my_specs.sc
    input_shape = tuple(np.array(input_shape) * np.array([sc, 1, 1]))
    ...
    tensors = [Input(shape=input_shape), ]
    tensors.append(AveragePooling3D(pool_size=(sc, 1, 1), strides=(sc, 1, 1), padding='valid')(tensors[-1]))
    for l, c in zip(up_model['layers'], up_model['connectivity']):
        ...
        #print(len(tensors))
    model = Model(inputs=tensors[0], outputs=tensors[-1])
...
def learn_without_groundtruth_simulated(input_shape, my_specs, lr):
    ...
    tensors = [Input(shape=input_shape),]
    tensors.append(AveragePooling3D(pool_size=(sc, 1, 1), strides=(sc, 1, 1), padding='valid', name='downscaled_zyx')(
        tensors[-1]))
    ...
    tensors.append(AveragePooling3D(pool_size=(sc, 1, 1), strides=(sc, 1, 1), padding='valid', name='pred_avg')(
        tensors[-1]))
    predavg_idx_first = len(tensors)-1

    tensors.append(Permute((1, 3, 2, 4), name='permute_yzx_forward')(tensors[pred_idx_first]))
    tensors.append(AveragePooling3D(pool_size=(sc, 1, 1), strides=(sc, 1, 1), padding='valid',
                                    name='downscaled_yzx_before')(tensors[-1]))
    ...
    for l, c in zip(up_model['layers'], up_model['connectivity']):
        if isinstance(c, int):
            ...
            raise ValueError("type should be int or tuple")
    tensors.append(AveragePooling3D(pool_size=(sc, 1, 1), strides=(sc, 1, 1), padding='valid',
                                    name='downscaled_yzx_after')(tensors[-1]))
    ...
    tensors.append(AveragePooling3D(pool_size=(sc, 1, 1), strides=(sc, 1, 1), padding='valid',
                                    name='downscaled_xyz_before')(tensors[-1]))
    ...
    for l, c in zip(up_model['layers'], up_model['connectivity']):
        ...
        if isinstance(c, int):
            ...
            raise ValueError("type should be int or tuple")
    tensors.append(AveragePooling3D(pool_size=(sc, 1, 1), strides=(sc, 1, 1), padding='valid',
                                    name='downscaled_xyz_after')(tensors[-1]))
    ...
    tensors.append(add([tensors[-1], tensors[pred_idx_first]], name='diff_gt'))
    diff_idx_gt = len(tensors)-1
    model = Model(inputs=tensors[0],
                  outputs=[tensors[diff_idx_1_2], tensors[diff_idx_1_3], tensors[diff_idx_avg_first],
...
def learn_from_groundtruth(input_shape, my_specs, lr):
    ...
    layers.append(AveragePooling3D(pool_size=(sc, 1, 1), strides=(sc, 1, 1), padding='valid')(layers[-1]))
